...
methods: {
  ...
  async deleteJukebox(jukeId) {
    try {
      const confirmed = await LNbits.utils.confirmDialog('Are you sure you want to delete this Jukebox?').onOk();
      if (confirmed) {
        this.JukeboxLinks = this.JukeboxLinks.filter(obj => obj.id !== jukeId);
      }
    } catch (error) {
  ...
  async submitSpotifyKeys() {
    ...
    try {
      const response = await LNbits.api.request('POST', '/jukebox/api/v1/jukebox/', this.g.user.wallets[0].adminkey, this.jukeboxDialog.data);
      if (response.data) {
  ...
  async getSpotifyTokens() {
    ...
    const timerId = setInterval(async () => {
      ...
      try {
        const response = await LNbits.api.request('GET', `/jukebox/api/v1/jukebox/${this.jukeboxDialog.data.sp_id}`, this.g.user.wallets[0].adminkey);
            ...
            setTimeout(() => {
              if (this.devices.length < 1 || this.playlists.length < 1) {
                this.$q.notify({
                  spinner: true,
                  color: 'red',
                  message: 'Error! Make sure Spotify is open on the device you wish to use, has playlists, and is playing something',
                  timeout: 10000
                });
                LNbits.api.request('DELETE', `/jukebox/api/v1/jukebox/${response.data.id}`, this.g.user.wallets[0].adminkey);
                this.getJukeboxes();
                clearInterval(timerId);
  ...
  requestAuthorization() {
    ...
    const redirectUri = encodeURI(`${this.locationcbPath}${this.jukeboxDialog.data.sp_id}`);
    window.open(`${url}?client_id=${this.jukeboxDialog.data.sp_user}&response_type=code&redirect_uri=${redirectUri}&show_dialog=true&scope=${scope}`);
  },
  ...
  async playlistApi(method, url, body) {
    try {
      ...
      if (response.status === 401) {
        ...
        await this.playlistApi('GET', 'https://api.spotify.com/v1/me/playlists', null);
        return; // Exit to prevent further processing in case of token refresh
      }

      const responseObj = await response.json();
      this.jukeboxDialog.data.playlists = responseObj.items.map(item => `${item.name}-${item.id}`);
  ...
  async deviceApi(method, url, body) {
    try {
      ...
      const responseObj = await response.json();
      this.jukeboxDialog.data.devices = responseObj.devices.map(device => `${device.name}-${device.id}`);
  ...
  async fetchAccessToken(code) {
    ...
    body += `&code=${code}`;
    body += `&redirect_uri=${encodeURI(this.locationcbPath + this.jukeboxDialog.data.sp_id)}`;
    ...
    try {
      const response = await fetch('https://accounts.spotify.com/api/token', {
        ...
        headers: {
          ...
          'Authorization': 'Basic ' + btoa(`${this.jukeboxDialog.data.sp_user}:${this.jukeboxDialog.data.sp_secret}`),
        },
        body: body,
      });
